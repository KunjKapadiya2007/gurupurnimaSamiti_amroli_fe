...
function Template() {
    ...
    console.log(acharyas , '111111111111111111111');
    ...
    function handleachary() {
        axios.get('https://guru-amroli-be.onrender.com/api/acharyas')
            .then((response) => setAcharyas(response.data))
            .catch((error) => console.log(error));
    }

    function handleAdhayax() {
        axios.get('https://guru-amroli-be.onrender.com/api/adhyakshs')
            .then((response) => setAdhayax(response.data))
            .catch((error) => console.log(error));
    }

    function handleGet() {
        axios.get('https://guru-amroli-be.onrender.com/api/samitis')
                    ...
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <Box sx={{ backgroundColor: '#FFF', borderRadius: '25px', m: 3, width: '100%' }}>
                            {acharyas.map((item, index) => (
                                <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }} key={index}>
